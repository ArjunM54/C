/*
2.There are total n number of Monkeys sitting on the branches of a huge Tree.
As travelers offer Bananas and Peanuts, the Monkeys jump down the Tree. If every Monkey can 
eat k Bananas and j Peanuts. If total m number of Bananas and p number of Peanuts are offered by 
travelers, calculate how many Monkeys remain on the Tree after some of them jumped down to eat.
...
on the ground or less than j Peanuts left on the ground, only that Monkey can eat Bananas(<k) 
...
{
    int n,k,j,m,p;
    printf("Enter the numbers of monkey in tree: ");
    scanf("%d",&n);
    ...
    int remain = n - (mp+mb+extra);
    printf("THE REMANING MONKEY IS: %d",remain);
}